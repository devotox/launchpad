...
export async function createDefaultConfig(user: {
  ...
  const orgName = emailDomain ? emailDomain.split('.')[0] : 'workspace';

  // Try to get team-specific workspace configuration
  let workspacePath = join(homedir(), "Documents", orgName || 'workspace');
  ...
  try {
    ...
    // If team has a specific workspace configuration, use it
    if (team?.config?.workspacePrefix) {
      workspacePath = join(homedir(), "Documents", team.config.workspacePrefix);
    }
  } catch {
    // If DataManager fails, use the email-derived path
  }
...
export async function validateConfigWithTeams(config: unknown): Promise<{ isValid: boolean; errors: string[] }> {
  ...
  const validConfig = config as LaunchpadConfig;
