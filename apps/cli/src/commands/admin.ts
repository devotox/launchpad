...
export class AdminCommand {
    ...
    try {
      ...
      if (providerToUse === 'github') {
        ...
        if (!configToUse.gistId) {
          ...
          console.log(chalk.gray('To download from a specific gist: --gist-id username/gistId'));
          console.log(chalk.gray('  (Also supports: gistId or https://gist.github.com/username/gistId)'));
          console.log(chalk.gray('To configure a default gist: launchpad admin config providers:add'));
          console.log(chalk.gray('To upload and create a new gist: launchpad admin config upload --token YOUR_TOKEN'));
          console.log(chalk.yellow("\n💡 Make sure your GitHub token has 'gist' scope for private gists"));
          return;
        }
      ...
      const gistConfig = await inquirer.prompt([
        {
          type: 'input',
          name: 'gistId',
          message: 'GitHub Gist ID (username/gistId, gistId, or URL - leave empty to create new):'
        },
        {
          type: 'input',
          name: 'fileName',
  ...
  async selectDefaultSyncProvider(): Promise<void> {
    ...
    await configManager.setDefaultSyncProvider(provider);
